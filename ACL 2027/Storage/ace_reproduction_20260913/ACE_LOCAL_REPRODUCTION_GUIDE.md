...
No experiment was started, interrupted, or reprioritized to create this guide. A subsequent source-path migration copied ACE dependencies into the project and updated the paused pilot configurations without changing scientific fields; see `reference_manifest.json` for provenance. Do not execute a second inference queue on Tom's machine while the existing GoEmotions queue is running.
...
- `snapshot/`: copies of 72 source, prompt, configuration, split-manifest, and upstream reference files, preserving repository-relative paths.
...
Raw corpus excerpts, model weights, the tokenizer cache, and prediction or annotation payloads are not included. Obtain the required data through the project's authorized distribution, preserving its bytes. All required ACE source dependencies now live inside `ACL 2027/ace_paper/`; the external sibling directory is not required. A Git clone by itself may omit local datasets and vendored files. Use `required_data_sha256` and `source_and_config_sha256` as the transfer inventory.
...
For **checkpoint continuation**, transfer a consistent stopped checkpoint, predictions, role caches, playbook, next bullet ID, annotations, and source snapshot. Do not copy a live adapting directory piecemeal and call it consistent. The historical paired configuration contains absolute path keys and fails direct equality after relocation. A documented path-only migration with semantic/hash verification is required before resuming that checkpoint. The same-machine migration from the external ACE source directory to `ACL 2027/ace_paper/` has been applied with original configurations archived in `Storage/ace_path_migration_20260913/`. A different-machine relocation still requires its own verified path-only migration.
