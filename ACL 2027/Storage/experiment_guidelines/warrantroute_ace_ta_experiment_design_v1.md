...
## Superseded objective: 2026-09-09

The user clarified that the intended task is detection of flaws in a supplied
claim, with an evolving Playbook and measured detection gains across development
checkpoints. Source-only TA generation is not that task. The v6 execution and
its heartbeat were stopped/paused; existing artifacts remain historical records.
Do not launch or resume this design as the user's claim-detection experiment.
See the [corrected claim-detection learning-curve protocol](warrantroute_claim_detection_learning_curve_v1.md).
The text below is retained to explain the superseded run, not to override the
corrected objective.
