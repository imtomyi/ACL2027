...
def adapt(native,q,value,memory,next_id,step,total,reference):
    ...
    try:tags=json.loads(raw).get('bullet_tags',[])
...
def freeze():
    ...
    cfg=dict(methods=METHODS,teacher=TEACHER,task=g.TASK,output_schema=OUTPUT_SCHEMA,codebooks=g.TOPICS,
        ...
        caps=dict(teacher=1024,ace_generator=1024,ace_reflector=1536,ace_curator=2048,ace_playbook=8000,dc_generator=2048,dc_curator=2048,dc_format_repairs=2,judge=384),
