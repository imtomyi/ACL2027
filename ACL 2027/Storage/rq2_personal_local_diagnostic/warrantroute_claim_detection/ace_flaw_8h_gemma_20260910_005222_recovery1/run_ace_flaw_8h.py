...
OPTIONS = {"temperature": 0, "seed": 20260910, "num_ctx": 32768, "num_predict": 4096}
...
def prepare(run, recover_from=None):
...
def execute_job(client, job, packet, memory):
    ...
    try:
        ...
        result.update(error=str(exc), technical_trajectory=True)
        if job["phase"] == "development":
            result["learning_status"] = "technical_failure"
        else:
            result["quality_status"] = "technical_failure"
