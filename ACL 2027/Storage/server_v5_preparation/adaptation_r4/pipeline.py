...
def write(status,**extra):
    value=dict(status=status,at=time.time(),active_stage='adaptation_r4',predecessor='r3 DC only; ACE resumes unchanged',do_not_resume_predecessor=True,**extra)
...
try:
    ...
    for action in ['preflight','seal']:
        ...
        if action=='preflight':
            ...
            if v['passed']!=6:raise RuntimeError('Development preflight failed; production remains unadmitted')
        if action=='seal':
            (root/'stages/adaptation_r3/DC_SUPERSEDED_BY.json').write_text(json.dumps(dict(stage=str(stage),at=time.time(),scope='DC only; r3 ACE checkpoints remain current'))+'\n')
    write('joint_launch')
    subprocess.run([py,str(root/'joint_adaptation/controller.py')],env=env,check=True)
