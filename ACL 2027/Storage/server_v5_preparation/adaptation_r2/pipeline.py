...
stage=root/'stages/adaptation_r2'
...
def write(status,**extra):
    value=dict(status=status,at=time.time(),active_stage='adaptation_r2',predecessor='adaptation_r1',do_not_resume_predecessor=True,**extra)
...
try:
    ...
    while True:
        ...
        live=[]
        for path in (root/'stages/adaptation_r1/runs').glob('*/worker.json'):
    ...
    for action in ['preflight','seal','launch']:
        ...
        if action=='seal':
            (root/'stages/adaptation_r1/SUPERSEDED_BY.json').write_text(json.dumps(dict(stage=str(stage),at=time.time(),scope='ACE/DC; historical r1 artifacts preserved'))+'\n')
