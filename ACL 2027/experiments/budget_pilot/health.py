...
ROOT=Path(__file__).resolve().parents[2];RUN=ROOT/'Storage/budget_pilot'/os.environ.get('EXPERIMENT_RUN_ID','run_20260913');MON=ROOT/'Storage/experiment_monitor'
...
def main():
    ...
    online=entry.get('phase')=='online'
    d=Path(entry['run_directory']) if entry.get('run_directory') else RUN/c if c else RUN;q=read(d/'queue.json');method=q.get('method');stage=q.get('stage')
    md=d/method if (online or c in ('goemotions','dreaddit')) and method else d/c/method if c and method and method!='reference_annotator' else d/'references'
    ...
    supervisor_alive=matches(sup.get('pid'),sup.get('script',str(ROOT/'experiments/budget_pilot/supervise.py')))
    ...
    policy=read(ROOT/'Storage/online100'/os.environ.get('EXPERIMENT_RUN_ID','run_20260913')/'policy.json')
    effective_deadline=None if policy.get(entry.get('phase','offline')+'_priority')=='complete_100' else entry.get('deadline')
    health=dict(observed_at=now,protocol='online100-v1' if online else 'budget-pilot-v1',phase=entry.get('phase','offline'),key=key,count_key=count,corpus=c,method=method,stage=stage,n=n,total=q.get('total'),classification=status,process_matches=child_alive,supervisor_alive=supervisor_alive,completed_calls=calls,last_confirmed_progress_at=last,no_progress_seconds=idle,median_seconds=median,ollama=server,deadline=effective_deadline,corpus_outcomes={k:v.get('status') for k,v in sup.get('corpora',{}).items()})
