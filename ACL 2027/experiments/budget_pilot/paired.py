...
CORPUS=os.environ.get('PILOT_CORPUS','cache')
assert CORPUS in ('cache','parlamint_gb')
RUN=ROOT/'Storage/budget_pilot/run_20260913'/CORPUS
g.TOPICS={CORPUS:g.TOPICS[CORPUS]}
g.RUN=RUN
...
def run_method(c,m):
    ...
    if offline:
        ...
        for i,row in enumerate(rows[s['adapted']:],s['adapted']):
            g.checkpoint(c,m,'adaptation',i,4);started=time.time();q=g.question(c,row)
            response,value=g.generate(native,q,s['memory'])
            reference=get_reference(c,'adapt',i,row) if has_ref else None
            s['memory'],s['next_id'],status=adapt(native,q,value,s['memory'],s['next_id'],i+1,4,reference)
    ...
    for i,row in enumerate(rows[s['n']:],s['n']):
        g.checkpoint(c,m,'prediction',i,8);started=time.time();q=g.question(c,row);before=hashlib.sha256(s['memory'].encode()).hexdigest()
        reference=None
        if dc:
        ...
        if dc:s['memory']=new_memory
            ...
            s['memory'],s['next_id'],status=adapt(native,q,value,s['memory'],s['next_id'],i+1,8,reference)
        s['n']=i+1;s['status']='complete' if i==7 else 'running';s['updated_at']=time.time()
        s['recent_seconds']=([time.time()-started] if i==0 else s['recent_seconds']+[time.time()-started])[-20:]
        g.atomic(p,s);g.checkpoint(c,m,'prediction',i+1,8);render()
        print(json.dumps(dict(corpus=c,method=m,n=i+1,total=8,seconds=time.time()-started)),flush=True)
...
def render():
    ...
    lines=['# Paired reference-feedback pilot','',
        ...
        f'Reference annotation preparation: {annotation_n}/12. Sealed: {(RUN/"references/sealed.json").exists()}.','',
    ...
    for c in g.TOPICS:
        for m in METHODS:
            ...
            conf=f'{100*passed/8:.2f}' if len(items)==8 and not unresolved else '—';agreement='—';invalid=0
            if s.get('status')=='complete' and s.get('n')==8 and (RUN/'references/sealed.json').exists():
                correct=0
                for i in range(8):
                ...
                agreement=f'{100*correct/8:.2f}'
            gt='—' if m=='base' else '✓†' if m.endswith('_ref') else '✗'
            lines.append(f"| {'CaChe' if c=='cache' else 'ParlaMint-GB'} | {DISPLAY[m]} | {gt} | N/A | {agreement} | {conf} | {s.get('status','queued')}; adapted {s.get('adapted',0)}; predictions {s.get('n',0)}/8; judged {len(items)}/8; unresolved {unresolved}; invalid topic outputs {invalid} |")
    lines+=['','† References are generated once from the source and fixed codebook, with no evaluated answers, playbooks or judge outcomes. All 12 annotations are sealed before comparative prediction. Matching codebook IDs and verbatim evidence checks verify format/traceability only, not semantic correctness.',
...
def freeze():
    ...
    cfg=dict(methods=METHODS,teacher=TEACHER,task=g.TASK,output_schema=OUTPUT_SCHEMA,codebooks=g.TOPICS,
        ...
        protocols=dict(offline='4 adaptation, 8 evaluation; no eval labels in offline adaptation',online='8 ordered examples, labels only after prediction',
        metrics='Reference topic-set agreement only, not human ACC; invalid output counts as nonagreement; final conformability requires 8 judgments, zero unresolved',
...
def audit(c,m):
    d=RUN/c/m;native=Native(d/'quality_calls');p=d/'quality.jsonl';items=g.read(p);rows=g.read(RUN/'data'/(c+'_test.jsonl'))
    for idx,row in enumerate(rows[len(items):],len(items)):
        g.checkpoint(c,m,'conformability',idx,len(rows));started=time.time()
        pred=json.loads((d/f'prediction_{idx:04d}.json').read_text());assert pred['record_id']==row['record_id']
        value=None;decision=None;error=None
        for attempt in range(3):
            try:
                raw=native.chat([dict(role='system',content=g.JUDGE+'\nCodebook: '+json.dumps(g.TOPICS[c])),dict(role='user',content=json.dumps(dict(source_passage=row['text'],anonymous_answer=pred['response'])))],max_tokens=384,role='quality_judge',output_format=g.SCHEMA)
                value=json.loads(raw);decision=g.decide(value);error=None;break
            except Exception as exc:error=type(exc).__name__
        item=dict(record_id=row['record_id'],decision=decision,judgment=value,error=error,seconds=time.time()-started)
        g.append(p,item);items.append(item)
        g.atomic(d/'quality_state.json',dict(n=len(items),total=len(rows),passed=sum(x['decision'] is True for x in items),failed=sum(x['decision'] is False for x in items),unresolved=sum(x['decision'] is None for x in items),recent_seconds=[x['seconds'] for x in items[-20:]]));render()
...
def main():
    ...
    g.render=render
    try:
        g.checkpoint(None,None,'freezing',0,12);freeze();render();prepare_references()
        for m in METHODS:
            for c in g.TOPICS:run_method(c,m);audit(c,m)
