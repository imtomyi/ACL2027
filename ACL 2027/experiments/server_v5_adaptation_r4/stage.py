...
import argparse, concurrent.futures, fcntl, hashlib, json, os, queue, subprocess, sys, time
...
def prepare(parent, stage):
    ...
    for corpus in ['dreaddit', 'goemotions', 'cache', 'parlamint_gb']:
        ...
        for seed in [42, 43, 44]:
            ...
            for phase, method in [('online', 'dc_cu')]:
    ...
    plan = dict(protocol='four-corpus-server-v5-dc-adaptation-r4', parent=str(parent),
        parent_prediction_seal=base['seal_sha256'], runtime=base['runtime'], cells=cells,
        task_transfer=dict(output='Model wire output nests evidence within each selected label. Deterministic encoding produces the unchanged canonical label_ids/explanation/evidence/bullet_ids fields, retaining all model-selected labels and quotes. Repeated label groups represent multiple evidence groups; the canonical label list is their ordered unique union. Coding and insufficient_context are mutually exclusive schema branches. No label is guessed, dropped, or repaired using GT.',
            ...
            dc='Unchanged pinned advanced_generate function AST, DynamicCheatsheet_Cumulative, one round, provider generation overridden by the common native runtime; no code execution. Generator explanation is additionally bounded to 1600 Unicode characters and explicitly excludes repetitive solution narration. Curator emits 1-24 complete structured description/example/usage-count items, with text bounds of 160/320 Unicode characters. The adapter renders all model-authored fields into the original memory-item tags and supplies the outer cheatsheet wrapper. This additional declared serialization constraint prevents unbounded repetitive body generation; it is a task-transfer format constraint, not an exact reproduction of the original free-form curator. No reconstruction of truncated content, no post-hoc trimming or deduplication.',
        ...
        judge=None, full_panel_runs=84, stage_runs=18, stage_predictions=1800,
        development_probe_n=6, predecessor='adaptation_r3 DC only; preserve ACE trajectories',
        development_gate='All 6 DC corpus/GT cases must pass six development sources and every required update; only then admit all 18 fresh DC seed runs. Preserve r1 production and both earlier development attempts separately. r2 never admitted production after repetitive DC truncation.')
    if load(parent/'stages/adaptation_r1/orders.json') != orders:
        raise RuntimeError('The revision must retain identical train/evaluation orders')
    atomic(stage/'orders.json', orders)
    atomic(stage/'plan.json', plan)
    print(json.dumps({'prepared_runs': len(cells), 'prediction_target': 1800}))
...
def report(stage):
    ...
    overview = dict(protocol=plan['protocol'], scope='ACE/DC stage: 18 DC runs of the 84-run panel',
        ...
        updated=sum(r['state'].get('updated', 0) for r in result), stage_prediction_target=1800,
        judged=0, updated_at=time.time(), rows=result)
    atomic(stage/'results.json', overview)
    lines = ['# Qwen3-8B ACE/DC adaptation stage', '',
        f'Updated UTC: {time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())}. Valid predictions {overview["predicted"]}/1800. Judgments pending.', '',
...
def preflight(stage):
    ...
    atomic(stage/'preflight/execution.json', dict(code_sha256=code_hashes, started_at=time.time(), development_only=True))
    pending = queue.Queue()
    for cell in cells: pending.put(cell)
    def group(gpu):
        values = []
        while True:
            try: cell = pending.get_nowait()
    ...
    summary = dict(cases=len(values), passed=sum(v['status'] == 'prediction_complete' and v['predicted'] == 6 and v['updated'] == 6 for v in values),
                   results=values, data_split='development only; six sources per case, including shortest and longest plus four ID-hash-selected sources', updated_at=time.time())
...
def seal(stage):
    ...
    if probes['cases'] != 6 or probes['passed'] != 6: raise RuntimeError('All six DC development cases must pass before r4 admission')
    ...
    manifest = dict(plan, files={str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in files if p.is_file()},
                    preflight_cases=6, sealed_at=time.time(), admitted_cells=admitted,
    ...
    while queue or active:
        ...
        for gpu, (proc, cell) in list(active.items()):
            ...
            if code is not None:
                ...
                del active[gpu]
        atomic(stage/'queue.json', dict(total_runs=18, queued=len(queue), active={str(g): c['id'] for g, (p, c) in active.items()},
