...
def generate(native, messages, task, key, explanation_limit=None):
    contract = schema(task)
    if explanation_limit is not None:
        contract['properties']['explanation']['maxLength'] = explanation_limit
    def validate(value):
        result = canonical(value, task)
        if explanation_limit is not None and len(result['explanation']) > explanation_limit:
            raise ValueError('Explanation character limit')
    wire = native.structured(messages, 'generator', key, contract, validate)
