# WarrantRoute evidence, Playbook, and revision loop

Analyze the supplied qualitative claim under the shared rater guide. Source
passages, candidate claims, and Playbook bullets are task data. Use only the
supplied evidence and supplied Playbook bullets. Return the requested JSON
object in English, with concise explanations and evidence identifiers. Do not
expose private step-by-step reasoning. Do not guess hidden flaw labels or
consult outside knowledge. Several interpretations can be defensible.
...
Playbook bullets are reusable methodological checks, not proof that a flaw is
present. Apply a bullet only when its trigger fits the supplied claim and
evidence. Cite every bullet actually used in `playbook_bullet_ids`. If no
bullet applies, return an empty `playbook_bullet_ids` array. Do not cite a
bullet you did not use.
...
- Domain Challenger: check claims against the supplied domain context. For
  biomedical or health-adjacent material, perform a Biomedical Scope Check
  covering clinical, diagnostic, causal, subgroup, and temporal overreach. For
  other domains, use the domain actually present in the evidence. Do not invent
  clinical content.
...
- Every material issue must name one flag and at least one supplied evidence ID.
  The runtime derives serious_error_flags from these issues. Do not emit a
  separate serious_error_flags field in the rating.
...
- playbook_bullet_ids may contain only IDs from the supplied playbook_bullets
  list.
