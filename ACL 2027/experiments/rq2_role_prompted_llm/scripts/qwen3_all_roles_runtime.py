#!/usr/bin/env python3
"""Hardened source-free runtime for the Qwen3 All roles component."""
...
class AllRolesRuntimeError(RuntimeError):
    """A finite, content-free runtime failure safe to report."""


class ResponseValidationError(AllRolesRuntimeError):
    """A model response failed the frozen response contract."""
...
def require(condition: bool, code: str) -> None:
    if not condition:
        raise AllRolesRuntimeError(code)
...
def read_regular_bytes(path: Path) -> bytes:
    ...
    try:
        ...
        raise AllRolesRuntimeError("required_file_unavailable") from exc
    try:
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise AllRolesRuntimeError("invalid_json_file") from exc
...
def validate_evaluator_item(item: dict[str, Any], schema: dict[str, Any]) -> None:
    try:
        ...
        raise AllRolesRuntimeError("evaluator_item_schema_invalid") from exc
    require(
...
def inert_canary_item() -> dict[str, Any]:
    """Return one in-memory interface fixture containing no corpus material."""
...
def api_json(
    ...
    *,
    ...
    try:
        ...
        raise AllRolesRuntimeError("local_model_service_unavailable") from exc
    try:
        value = json.loads(body)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise AllRolesRuntimeError("local_model_service_invalid_json") from exc
