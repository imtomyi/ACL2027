...
DEFAULT_CONFIG = ROOT / "config/warrantroute_loop_n100_same_model_v1.json"
...
def plan(args: argparse.Namespace) -> tuple[dict, dict, dict, dict | None]:
    ...
    for name in names:
        assignment = config["configurations"][name]
        if set(assignment) != set(runtime.ROLES):
            raise ValueError("configuration_must_assign_every_agent_role")
        for route in runtime.MODE_ROLES:
            runtime.validate_model_family_separation({
                "route_mode": route,
                "agent_assignments": [{"agent_id": role, "role": role, "model_id": model,
                                       "model_family": model.split(":")[0]}
                                      for role, model in assignment.items()],
            }, policy=config.get("model_assignment_policy", "cross_family"))
    ...
    playbook_bundle = runtime.load_playbook_bundle(config)
    ...
    manifest = {
        ...
        "model_assignment_policy": config.get("model_assignment_policy", "cross_family"),
        "cell_canary_first": config.get("cell_canary_first", False),
        "implementation": {str(path): sha_file(path) for path in implementation_files()},
        "manuscript_eligible": False, "playbook_updates": False,
        "playbook": runtime.playbook_summary(playbook_bundle),
    ...
    return config, manifest, packets, playbook_bundle


def prepare(args: argparse.Namespace) -> dict:
    config, manifest, packets, playbook_bundle = plan(args)
    ...
    if playbook_bundle:
        write_json(run_dir / "playbook_bundle.json", playbook_bundle)
        manifest["assets"]["playbook_bundle.json"] = sha_file(run_dir / "playbook_bundle.json")
            ...
            for name in manifest["configurations"]:
                for method in manifest["methods"]:
                    for index, packet in enumerate(rows):
                        ordered.append((index, (dataset, seed, name, method, packet)))
    if manifest.get("cell_canary_first", False):
        ordered.sort(key=lambda item: item[0] != 0)
    for _index, job in ordered:
        yield job
...
def execute(args: argparse.Namespace) -> dict:
    ...
    with (run_dir / "run.lock").open("a") as lock:
        ...
        playbook_bundle = read_json(run_dir / "playbook_bundle.json") if (run_dir / "playbook_bundle.json").exists() else None
        runtime.validate_playbook_bundle(playbook_bundle) if playbook_bundle else None
        completed = 0
        failures = 0
        for dataset, seed, name, method, packet in jobs(manifest, packets):
            ...
            if playbook_bundle:
                client = runtime.AgentClient(config, config["configurations"][name], seed, prompt, guide,
                                             directory / "calls", write_json, runtime.ollama_transport,
                                             playbook_bundle, packet["corpus_id"])
            else:
                client = runtime.AgentClient(config, config["configurations"][name], seed, prompt, guide,
