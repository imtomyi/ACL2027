"""Prepare, preflight, seal, dispatch and report a separate configurable ACE/DC stage."""
...
from sizes import cell_sizes, panel_target
...
def prepare(parent, stage):
            ...
            for phase, method in [('offline', 'ace'), ('online', 'ace'), ('online', 'dc_cu')]:
                for gt in (['yes', 'no'] if corpus in ['dreaddit', 'goemotions'] else ['no']):
                    cells.append(dict(id=f'{corpus}__{phase}__{method}__gt_{gt}__seed{seed}',
                        dataset=corpus, phase=phase, method=method, adaptation_gt=gt, seed=seed, **cell_sizes(parent,corpus,phase,method)))
    ...
    plan = dict(protocol='four-corpus-server-v5-configurable-adaptation', parent=str(parent),
        ...
        task_transfer=dict(output='Model wire output nests evidence within each selected label. Deterministic encoding produces the unchanged canonical label_ids/explanation/evidence/bullet_ids fields, retaining all model-selected labels and quotes. Repeated label groups represent multiple evidence groups; the canonical label list is their ordered unique union. Coding and insufficient_context are mutually exclusive schema branches. No label is guessed, dropped, or repaired using GT.',
            ...
            dc='Unchanged pinned advanced_generate function AST, DynamicCheatsheet_Cumulative, one round, provider generation overridden by the common native runtime; no code execution. Curator emits 1-24 complete structured description/example/usage-count items, with text bounds of 160/320 Unicode characters. The adapter renders all model-authored fields into the original memory-item tags and supplies the outer cheatsheet wrapper. This additional declared serialization constraint prevents unbounded repetitive body generation; it is a task-transfer format constraint, not an exact reproduction of the original free-form curator. No reconstruction of truncated content, no post-hoc trimming or deduplication.',
        ...
        judge=None, full_panel_runs=84, stage_runs=54, stage_predictions=sum(c['n'] for c in cells), stage_update_target=sum(c['update_n'] for c in cells), full_panel_prediction_target=panel_target(parent),
        development_probe_n=6, predecessor='adaptation_r3; fresh-size successor, no trajectory reuse',
        development_gate='All 18 corpus/phase/method/GT cases must pass six development sources and every required update; only then admit all 54 fresh seed runs. Preserve r1 production and both earlier development attempts separately. r2 never admitted production after repetitive DC truncation.')
...
def report(stage):
    ...
    overview = dict(protocol=plan['protocol'], scope='ACE/DC stage: 54 runs of the 84-run panel',
        ...
        updated=sum(r['state'].get('updated', 0) for r in result), stage_prediction_target=plan['stage_predictions'],
    ...
    for r in result:
        ...
        lines.append(f'| {c["dataset"]} | {c["phase"]} | {c["method"]} | {c["adaptation_gt"]} | {c["seed"]} | {s.get("predicted",0)}/{c["n"]} | {s.get("updated",0)}/{c["update_n"]} | {acc} | {fmt("micro_f1")} | {fmt("macro_f1")} | PENDING | {s.get("active_seconds",0)/60:.1f} | {s["status"]} |')
...
def seal(stage):
    ...
    if probes['cases'] != 18 or probes['passed'] != 18: raise RuntimeError('All development cases must pass before r3 admission')
    ...
    while queue or active:
        ...
        for gpu, (proc, cell) in list(active.items()):
            ...
            if code is not None:
                ...
                del active[gpu]
        atomic(stage/'queue.json', dict(total_runs=len(manifest['cells']), queued=len(queue), active={str(g): c['id'] for g, (p, c) in active.items()},
