...
from core import Native, atomic, load, rows, digest, evidence_checks
...
def execute(parent, stage, cell, native, upstream, dev=False, probe_n=6):
    ...
    def status(kind, error=None):
        ...
        value = dict(condition_id=cell['id'], dataset=cell['dataset'], method=cell['method'],
                     ...
                     updated=len(updates), total=probe_n if dev else cell['n'], update_total=probe_n if dev else cell['update_n'], judged=0,
    ...
    try:
        ...
        for phase in phases:
            ...
            if dev:
                ...
                expected = cell['train_n'] if phase == 'adaptation' else cell['n']
                if len(order) != expected or len(data) != expected: raise RuntimeError('Dataset count differs from sealed manifest')
