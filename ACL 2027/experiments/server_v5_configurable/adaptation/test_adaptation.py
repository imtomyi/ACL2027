...
from output_contract import canonical, schema, validate_cheatsheet, render_cheatsheet
...
class NativeFixture:
    ...
    def chat(self, messages, role, key, schema=None, **kwargs):
        ...
        if role == 'generator':
            ...
            if 'memory_items' in schema['properties']:
                value = {'memory_items': [] if self.bad_dc else [{'description': 'Test memory.', 'example': 'A complete example.', 'usage_count': 1}]}
...
class WireBehavior(unittest.TestCase):
    ...
    def test_cheatsheet_body_requires_complete_structure(self):
        for bad in [{}, {'memory_items': []}, {'memory_items': [{}]},
                    {'memory_items': [{'description': 'x'*161, 'example': '', 'usage_count': 0}]}]:
            with self.assertRaises(ValueError): validate_cheatsheet(bad)
        validate_cheatsheet({'memory_items': [{'description': 'Complete.', 'example': '', 'usage_count': 0}]})

    def test_rendering_retains_fields_and_escapes_literal_tags(self):
        from html import unescape
        item = {'description': 'A <cheatsheet> tag is literal.', 'example': 'x < y & z', 'usage_count': 7}
        rendered = render_cheatsheet({'memory_items': [item, item]})
        self.assertNotIn('<cheatsheet>', rendered)
        self.assertEqual(rendered.count('<memory_item>'), 2)
        self.assertIn(item['description'], unescape(rendered))
        self.assertIn(item['example'], unescape(rendered))
        self.assertEqual(rendered.count('** Count: 7'), 2)

    def test_item_and_character_bounds_are_enforced_without_trimming(self):
        item={'description':'x'*160, 'example':'y'*320, 'usage_count':1000000}
        value={'memory_items':[item]*24}
        validate_cheatsheet(value)
        rendered=render_cheatsheet(value)
        self.assertEqual(rendered.count('y'*320),24)
        with self.assertRaises(ValueError): validate_cheatsheet({'memory_items':[item]*25})
        with self.assertRaises(ValueError): validate_cheatsheet({'memory_items':[dict(item, example='y'*321)]})

    def test_dc_explanation_bound_rejects_without_trimming(self):
        from output_contract import generate
        class LongExplanation(NativeFixture):
            def chat(self, messages, role, key, schema=None, **kwargs):
                value=json.loads(super().chat(messages,role,key,schema,**kwargs))
                if role=='generator':value['explanation']='x'*1601
                return json.dumps(value)
        n=LongExplanation()
        self.assertEqual(len(generate(n,[],TASK,'unbounded')['explanation']),1601)
        with self.assertRaisesRegex(ValueError,'Format retries exhausted'):
            generate(n,[],TASK,'bounded',explanation_limit=1600)
