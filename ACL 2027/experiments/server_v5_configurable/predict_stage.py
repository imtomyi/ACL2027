...
def run(root,cell,gpu):
    ...
    assert len(order)==cell['n']==len(test), 'Evaluation count differs from sealed manifest'
    ...
    def status(kind,error=None):
        ...
        value={'condition_id':cell['id'],'dataset':cell['dataset'],'method':cell['method'],'seed':cell['seed'],'phase':cell['phase'],'adaptation_gt':cell['adaptation_gt'],'status':kind,'attempted':len(outputs),'predicted':valid,'total':cell['n'],'judged':0,'judge_status':'pending_selection','gpu':gpu,'pid':os.getpid(),'active_seconds':base_elapsed+time.monotonic()-clock,'updated_at':time.time(),'prediction_seal_sha256':manifest['seal_sha256']}
