...
from sizes import counts, cell_sizes, panel_target, manifest as sampling_manifest
...
def main():
    ...
    gate=load(root/'preflight/report.json')
    if gate.get('status')!='complete':raise RuntimeError('Development preflight must pass before sealing')
    for relative, expected in sampling_manifest(root)['files'].items():
        if hashlib.sha256((root/relative).read_bytes()).hexdigest()!=expected:raise RuntimeError('Prepared sampling data changed')
    ...
    for c,task in tasks.items():
        test=rows(root/'data'/c/'test.jsonl');train=rows(root/'data'/c/'train.jsonl');assert len(test)==counts(root,c)['test'] and len(train)==counts(root,c)['train']
        ...
        for seed in [42,43,44]:
            ...
            for method in ['base']+(['icl'] if c in ['dreaddit','goemotions'] else []):
                cells.append({'id':f'{c}__{method}__seed{seed}','dataset':c,'method':method,'seed':seed,'phase':'baseline' if method=='base' else 'offline','adaptation_gt':'not_applicable' if method=='base' else 'yes',**cell_sizes(root,c,'baseline' if method=='base' else 'offline',method)})
    ...
    files={}
    for p in list((root/'code').glob('*.py'))+list((root/'data').rglob('*.json*'))+[root/'orders.json',root/'demonstrations.json',root/'tasks.json',root/'runtime.json',root/'AMENDMENTS.json',root/'sampling_manifest.json',root/'dataset_sizes.json',root/'preflight/report.json']:
    ...
    manifest={'protocol':'four-corpus-server-v5-configurable-prediction','parent_design':'four-corpus-server-v5','stage':'Base/ICL predictions only; other adapters and judge not represented as ready','launch_authorized':True,'cells':cells,'runtime':runtime,'files':files,'sample_counts':sampling_manifest(root)['counts'],'full_panel_prediction_target':panel_target(root),'seeds':[42,43,44],'full_panel_runs':84,'judge':None,'judge_policy':'Separate later seal, blind to methods and aggregate test performance; no judge feedback to generation','evidence_policy':'Raw model response retained; exact-quote occurrence and reported offset validity reported separately. Deterministic unique exact anchors do not alter label decisions or raw quotes. No fuzzy repair.','historical_score_reuse':False,'online_adaptation':False}
    manifest['seal_sha256']=digest(manifest);atomic(root/'prediction_manifest.json',manifest)
    print(json.dumps({'sealed_runs':len(cells),'predictions':sum(c['n'] for c in cells),'model_calls':0,'demo_tokens':{c:{s:v['tokens'] for s,v in d.items()} for c,d in demonstrations.items() if d}}))
