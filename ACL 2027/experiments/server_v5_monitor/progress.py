...
DEFAULT_ROOT = Path('/home/sy23985/Storage/acl2027_server_v5_20260914')
...
def snapshot(root, gpu=True):
    ...
    development = []
    development_target = 18
    development_probe_n = 6
    active_stage = transition.get('active_stage', '')
    if active_stage and '/' not in active_stage:
        dev_plan = read_json(root/'stages'/active_stage/'plan.json', warnings) or {}
        development_target = sum(c.get('seed') == 42 for c in dev_plan.get('cells', [])) or 18
        development_probe_n = dev_plan.get('development_probe_n', 6)
    ...
    return dict(stages=stages, rows=list(selected.values()), warnings=warnings,
                gpus=gpu_rows, transition=transition, development=development, development_target=development_target, development_probe_n=development_probe_n, at=time.time(), monotonic=time.monotonic())
...
def render(value, previous=None, show_all=False, max_rows=15):
    ...
    lines = ['ACL 2027 | Qwen3-8B | '+stamp,
        'Read-only live metadata | 100 evaluation items/corpus | 4 corpora | seeds 42/43/44', '',
    ...
    if transition:
        ...
        if probes and transition.get('status') in ['preflight', 'pipeline_stopped', 'seal']:
            passed = sum(s['status']=='prediction_complete' and s['predicted']==value['development_probe_n'] and s['updated']==value['development_probe_n'] for s in probes)
            failed_dev = sum(s['status']=='failed' for s in probes)
            lines.append('Development validation: {}/{} passed | {} active | {} failed | {}/{} predictions | {}/{} updates'.format(
                passed, value['development_target'], sum(s['status']=='running' for s in probes), failed_dev,
                sum(s.get('predicted',0) for s in probes), value['development_target']*value['development_probe_n'], sum(s.get('updated',0) for s in probes), value['development_target']*value['development_probe_n']))
    ...
    for stage in value['stages']:
        ...
        lines.append('  {:<20} pred {:>4}/{:<4}  updates {:>4}  pred-done {:>2}/{:<2}'.format(
            stage['name'][:20], amount(admitted, 'predicted'), len(admitted)*100, amount(admitted, 'updated'),
