...
def prepare(parent, stage):
    ...
    plan = dict(protocol='four-corpus-server-v5-adaptation-r2', parent=str(parent),
        ...
        task_transfer=dict(output='Model wire output nests evidence within each selected label. Deterministic encoding produces the unchanged canonical label_ids/explanation/evidence/bullet_ids fields, retaining all model-selected labels and quotes. Repeated label groups represent multiple evidence groups; the canonical label list is their ordered unique union. Coding and insufficient_context are mutually exclusive schema branches. No label is guessed, dropped, or repaired using GT.',
            ...
            dc='Unchanged pinned advanced_generate function AST, DynamicCheatsheet_Cumulative, one round, provider generation overridden by the common native runtime; no code execution. Curator emits structured JSON containing the complete memory body. The adapter supplies the outer tags to the unchanged upstream extractor; no reconstruction of a truncated or malformed memory.',
        ...
        development_probe_n=6, predecessor='adaptation_r1',
        development_gate='All 18 corpus/phase/method/GT cases must pass six development sources and every required update; only then admit all 54 fresh seed runs. Preserve all r1 predictions, failures and memories separately.')
...
def seal(stage):
    ...
    if probes['cases'] != 18 or probes['passed'] != 18: raise RuntimeError('All development cases must pass before r2 admission')
