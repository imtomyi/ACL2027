...
def development_schedule(data, plan):
    ...
    for epoch in range(1, plan["epochs"] + 1):
        ...
        for position in range(plan["selection"]["development_n"]):
            for ds in plan["datasets"]:
...
def expansion_inventory(source, identity):
    old, final = read(source / "manifest.json"), read(source / "final_manifest.json")
    require(final["status"].startswith("completed_"), "expansion_source_not_terminal")
    require(final["core_processed"] == final["core_expected"], "expansion_source_incomplete")
    for manifest in (old, final):
        for name, expected in manifest["files"].items():
            require(file_hash(source / name) == expected, "expansion_source_changed:" + name)
    require(old["model_digest"] == identity, "expansion_model_changed")
    require(read(source / "runtime_config.json")["options"] == OPTIONS, "expansion_decoding_changed")
    require(read(source / "prompts_and_schemas.json") == {"prompts": contract.PROMPTS, "schemas": contract.SCHEMAS},
            "expansion_prompts_changed")
    require(file_hash(source / "ace_flaw_contract.py") == file_hash(Path(__file__).with_name("ace_flaw_contract.py")),
            "expansion_semantic_contract_changed")
    require(all(p.with_name("response.json").exists() for p in (source / "calls").glob("**/request.json")),
            "expansion_source_ambiguous_call")
    return {"source_run": str(source), "source_manifest_sha256": file_hash(source / "manifest.json"),
            ...
            "same_prompts_schemas_semantic_contract_model_decoding": True,
            "imported_outputs_references_memory": 0, "prior_packet_ids_text_records_excluded": True,
            "historical_other_protocol_exposure_possible": True,
            "authorization": "User requested more data under the same experiment on 2026-09-10.",
            "changes": "Allocation, source exclusion, count-aware scheduling/export, and launch verification only."}


def prepare(run, recover_from=None, restart_from=None, smoke_report=None, expansion_from=None):
    require(run.is_relative_to((PROJECT / "Storage").resolve()), "private_storage_only")
    require(sum(p is not None for p in (recover_from, restart_from, expansion_from)) <= 1,
            "recovery_restart_expansion_are_distinct")
    plan = design.expansion_plan(expansion_from) if expansion_from else read(design.PLAN)
    ...
    recovered, restart, expansion = None, None, None
    ...
    elif expansion_from is not None:
        require(smoke_report is not None and smoke_report.is_relative_to((PROJECT / "Storage").resolve()),
                "expansion_requires_private_live_smoke_gate_path")
        expansion = expansion_inventory(expansion_from, identity)
        require(all(m["name"] == MODEL for m in api("/api/ps").get("models", [])),
                "another_model_loaded_check_concurrent_work")
    else:
    ...
    if expansion is not None:
        config = read(run / "runtime_config.json")
        config.update(authorization_message=expansion["authorization"],
                      required_smoke_report=str(smoke_report), allocation_revision="new_dev10_eval80_v2")
        write_json(run / "runtime_config.json", config, replace=True)
        write_json(run / "expansion_provenance.json", expansion)
    ...
    if expansion is not None:
        shutil.copyfile(PROJECT / "Storage/experiment_guidelines/ace_flaw_expansion_20260910.md",
                        run / "expansion_protocol.md")
...
def restart_inventory(source, inputs, model_digest, smoke_report):
    ...
    validate_smoke_report(smoke_report, inputs, model_digest)
...
def validate_smoke_report(smoke_report, inputs, model_digest):
    ...
    require(len(checks) == 8 and all(c["diagnosis_status"] == "valid" and not c["technical_trajectory"]
            and c.get("quality_schema_valid") for c in checks), "smoke_checks_incomplete")
    expected_ids = {(ds, p["packet_id"]) for ds in DISPLAY
                    for p in sorted(inputs[ds]["development"], key=lambda p: digest([
                        20260910, "gemma8h-v6", ds, 1, p["packet_id"]]))[:2]}
    require({(c["dataset"], c["packet_id"]) for c in checks} == expected_ids,
            "smoke_packet_selection_changed")
...
class Client:
    ...
    def call(self, key, role, data):
        ...
        if request_path.exists():
            ...
            ceiling = read(plan_path)["expected"]["all_calls_max"] if plan_path.exists() else 1200
            if len(list((self.run / "calls").glob("**/request.json"))) >= ceiling:
...
def worker_locked(run):
    ...
    if not chosen_path.exists():
        ...
        for job in read(run / "development_schedule.json"):
            ...
            if result.get("phase_limited"):
                break
            if job["position"] == plan["selection"]["development_n"]:
    ...
    for epoch in (1, 2):
        ...
        if not all((run / "playbooks" / ds / f"E{epoch}.json").exists() for ds in plan["datasets"]):
            continue
        for position in range(plan["selection"]["optional_probe_n"]):
            for ds in plan["datasets"]:
    ...
    for job in optional:
        ...
        if result["memory_before_sha256"] != result["memory_after_sha256"]:
            raise FrozenError("optional_modified_memory")
        failures = failures + 1 if result["technical_trajectory"] else 0
        if failures >= 3:
            raise RuntimeError("three_consecutive_technical_trajectories")
        if result.get("phase_limited"):
...
def export(run, terminal=None):
    ...
    expected, selection = plan["expected"], plan["selection"]
    evaluation_n, probe_n = selection["evaluation_n"], selection["optional_probe_n"]
    ...
    for ds in plan["datasets"]:
        for epoch in sorted({0, final_epoch}):
            ...
            cell = quality_cell(items, evaluation_n)
            ...
            row = {"Dataset": DISPLAY[ds], "Method": METHOD, "Model": MODEL, "Checkpoint": label,
                   "Planned_N": evaluation_n, "Processed": cell["reviews_processed"], "Valid_reviews": cell["reviews_valid"]}
            for dim in ("credibility", "conformability"):
                q = cell[dim]
    ...
    probe_rows = []
    for ds in plan["datasets"]:
        probe = {p["packet_id"] for p in inputs[ds]["evaluation"][:probe_n]}
        for epoch in range(4):
            ...
            cell = quality_cell(selected_rows, probe_n)
            probe_rows.append({"Dataset": DISPLAY[ds], "Checkpoint": f"E{epoch}", "Planned_N": probe_n,
    ...
    lines = ["# Results Table: ACE-Inspired Flaw Detection", "", f"Updated: {now()}", "",
             ...
             f"| Dataset | Method | Checkpoint | Reviews / {evaluation_n} | Credibility | Conformability |", "| --- | --- | --- | ---: | --- | --- |"]
    ...
    lines.extend(["", "Full scheduled-panel rates, T/F/unknown/technical counts and coverage are in results_table_ACE.csv.",
                  ...
                  "CaChe is a within-source diagnostic. A checkpoint label does not mean all quality judgments are resolved.", ""])
    ...
    dev_left = 0 if chosen else max(0, expected["development_exposures"] - len(development))
    core_target = expected["unique_evaluation_packets"] if chosen and chosen["epoch"] == 0 else expected["core_evaluation_reviews"]
    eval_left = max(0, core_target - len(core))
    ref_left = max(0, expected["unique_packets"] - len(list((run / "references").glob("**/*.json"))))
    ...
    if terminal == "core_finished":
        ...
        elif valid_pairs != expected["core_evaluation_reviews"]:
    ...
    status = {"updated_at": now(), "state": state, "method": METHOD, "model": MODEL,
        "development_processed": len(development), "development_planned": expected["development_exposures"],
        "development_valid": sum(r["status"] == "valid" for r in development),
        "core_reviews_processed": len(core), "core_reviews_planned": expected["core_evaluation_reviews"],
    ...
    if terminal and terminal != "prepared_not_started":
        final = {"status": state, "at": now(), "table_rows": len(main_rows), "playbook_rows": len(memory_rows),
                 "core_expected": expected["core_evaluation_reviews"], "core_processed": len(core), "binary_pairs": valid_pairs,
...
def supervise_locked(run, manifest):
    ...
    gate = read(run / "runtime_config.json").get("required_smoke_report")
    if gate:
        validate_smoke_report(Path(gate), read(run / "inputs.private.json"), manifest["model_digest"])
        record = {"smoke_report": gate, "sha256": file_hash(Path(gate))}
        receipt = run / "launch_gate_receipt.json"
        if receipt.exists():
            require(read(receipt) == record, "launch_gate_changed")
        else:
...
def main():
    ...
    parser.add_argument("--expansion-from", type=Path, help="New disjoint dev10/eval80 allocation using unchanged v2 semantic contracts")
    ...
    if args.command == "prepare":
        result = prepare(run, args.recover_from.resolve() if args.recover_from else None,
                         ...
                         args.expansion_from.resolve() if args.expansion_from else None)
