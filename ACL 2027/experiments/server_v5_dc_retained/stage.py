...
def report(stage):
    ...
    overview = dict(protocol=plan['protocol'], scope='DC retained-update variant: three GoEmotions GT-yes seed runs',
    ...
    while queue or active:
        ...
        for gpu, (proc, cell) in list(active.items()):
            ...
            if code is not None:
                ...
                del active[gpu]
        atomic(stage/'queue.json', dict(total_runs=len(manifest['admitted_cells']), queued=len(queue), active={str(g): c['id'] for g, (p, c) in active.items()},
