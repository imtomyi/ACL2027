...
from output_contract import FORMAT, generate as generate_canonical, CHEATSHEET_SCHEMA, CURATOR_FORMAT, validate_cheatsheet, render_cheatsheet
from retained_policy import CuratorOutputRejected, retained_update
...
class ACE:
    ...
    def generate(self, source, memory, key):
        ...
        return generate_canonical(self.native, [{'role': 'user', 'content': prompt}], self.task, key)
            ...
            def generate(self, history, **kwargs):
                ...
                if self.calls == 1:
                    ...
                    if value is None:
                        ...
                        history[-1]['content'] += FORMAT + '\nFor this DC task adapter, explanation must be a complete, brief source-grounded justification of at most 1600 Unicode characters. Give the justification once; do not narrate repeated solution attempts.'
                        value = generate_canonical(adapter.native, history, adapter.task, key + ':generate', explanation_limit=1600)
                ...
                history[-1]['content'] += '\n\n' + feedback + CURATOR_FORMAT
                try:
                    raw = adapter.native.structured(history, 'curator', key + ':curate', CHEATSHEET_SCHEMA, validate_cheatsheet)
                except CuratorOutputRejected as exc:
                    self.update_record = retained_update(adapter.native, memory, next_id, exc.evidence)
                    return '<cheatsheet>' + memory + '</cheatsheet>'
                candidate = render_cheatsheet(raw)
                self.update_record = bounded(adapter.native, memory, candidate, next_id, next_id)
                self.update_record['curator_response'] = raw
                self.update_record['proposed_memory_items'] = len(raw['memory_items'])
                self.update_record['duplicate_memory_items'] = len(raw['memory_items']) - len({json.dumps(item, sort_keys=True) for item in raw['memory_items']})
