...
def scores(pred,gold,task):
    ...
    result={'prediction_n':n,'valid_n':len(valid),'correct':correct,'accuracy':100*correct/100 if n==100 else None,'valid_accuracy':100*correct/len(valid) if valid else None,'accuracy_status':'complete' if n==100 else 'partial'}
    if n!=100 or len(valid)!=100:return result
...
def report(root):
    ...
    overview={'protocol':manifest['protocol'],'scope':'First stage: 18 Base/ICL seed runs of the planned 84-run Qwen3-8B panel','predicted':total,'attempted':attempted,'stage_prediction_target':1800,'full_panel_prediction_target':8400,'judged':0,'updated_at':time.time(),'rows':values}
    atomic(root/'results.json',overview)
    lines=['# Qwen3-8B server prediction stage','',f'Updated UTC: {time.strftime("%Y-%m-%d %H:%M:%S",time.gmtime())}. Predicted {total}/1800 for this stage. Full planned panel: 84 runs / 8400 predictions. Judgments: pending.','',
      ...
      '| --- | --- | ---: | ---: | ---: | ---: | ---: | --- | ---: | --- |']
    for r in values:
        c,s,m=r['cell'],r['state'],r['metrics'];fmt=lambda k:f'{m[k]:.2f}' if m.get(k) is not None and s.get('attempted')==100 else '—'
        acc='N/A' if c['dataset'] in ['cache','parlamint_gb'] else fmt('accuracy')
        lines.append(f"| {c['dataset']} | {c['method']} | {c['seed']} | {s.get('predicted',0)}/100 | {acc} | {fmt('micro_f1')} | {fmt('macro_f1')} | PENDING | {s.get('active_seconds',0)/60:.1f} | {s['status']} |")
    ...
    return {'predicted':total,'target':1800,'judged':0}
